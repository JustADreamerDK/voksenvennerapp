...
<body>
    ...
    <section class="flex-column center p-b-175">
        ...
        <?php if ($rowDayTest['id'] <> '' && $type == '2' OR $type =='3'){ ?>
<?php
        $day = mysqli_query($objCon, "SELECT * FROM `dag` WHERE `venskabs_id` = '$venskabsId' LIMIT $start, $limit");
 ...
 ?>
        ...
        <div class="flex m-t-10 between w-80">
            ...
            if ($page > 1) {
                echo "<a href='?page=" . ($page - 1) . "'><h3 class='bladre'>Forrige</h3></a>";
            } else {
                echo "<div></div>";
            }
            ?>
            ...
            <?php
            if ($page != $total) {
                echo "<a href='?page=" . ($page + 1) . "'><h3 class='bladre'>Næste</h3></a>";
            } else {
                echo "<div></div>";
            }}
            ?>
